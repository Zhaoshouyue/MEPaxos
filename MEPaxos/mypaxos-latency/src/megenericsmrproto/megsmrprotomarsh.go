...
/*******************************************/
func (t *RepliPropose) Marshal(wire io.Writer) {
	var b [8]byte
	var bs []byte
	var bs1 []byte
	bs = b[:4]
	...
	bs1[0] = byte(tmp641)
	bs1[1] = byte(tmp641 >> 8)
	bs1[2] = byte(tmp641 >> 16)
	bs1[3] = byte(tmp641 >> 24)
	bs1[4] = byte(tmp641 >> 32)
	bs1[5] = byte(tmp641 >> 40)
	bs1[6] = byte(tmp641 >> 48)
	bs1[7] = byte(tmp641 >> 56)
	wire.Write(bs1)
}
/***********************************************/
...
/**************************************************************/
func (t *RepliPropose) Unmarshal(wire io.Reader) error {
	var b [8]byte
	var bs []byte
	bs = b[:4]
	if _, err := io.ReadAtLeast(wire, bs, 4); err != nil {
	...
	t.ConnToCli = int64((uint64(bs[0]) | (uint64(bs[1]) << 8) | (uint64(bs[2]) << 16) | (uint64(bs[3]) << 24) | (uint64(bs[4]) << 32) | (uint64(bs[5]) << 40) | (uint64(bs[6]) << 48) | (uint64(bs[7]) << 56)))
	return nil
}
/**************************************************************/
...
/*******************************************************/
func (t *ProposeRepliReplyTS) Marshal(wire io.Writer) {
	var b [8]byte
	var bs []byte
	var bs1 []byte
	bs = b[:5]
...
/***********************************************************/
func (t *ProposeRepliReplyTS) Unmarshal(wire io.Reader) error {
	var b [8]byte
	var bs []byte
	bs = b[:5]
	if _, err := io.ReadAtLeast(wire, bs, 5); err != nil {
...
/***********************************************************/
func (t *ProposeRepliReplyTS) New() fastrpc.Serializable {
	return new(ProposeRepliReplyTS)
}
/***********************************************************/
