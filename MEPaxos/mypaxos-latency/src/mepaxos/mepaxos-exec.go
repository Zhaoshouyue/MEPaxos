...
func (e *Exec) executeCommand(replica int32, instance int32) bool {
	...
	if inst.Status == mepaxosproto.EXECUTED {
		return true
	}
	if inst.Status != mepaxosproto.COMMITTED {
		return false
	}
	...
	*index = *index + 1
	...
	for q := int32(0); q < int32(e.r.N); q++ {
		...
		for i := e.r.ExecedUpTo[q] + 1; i <= inst; i++ {
			...
			*/
			if e.r.InstanceSpace[q][i].Status == mepaxosproto.EXECUTED {
				continue
			}
			for e.r.InstanceSpace[q][i].Status != mepaxosproto.COMMITTED {
				time.Sleep(1000 * 1000)
			}
	...
	if v.Lowlink == v.Index {
		...
		for _, w := range list {
			...
			w.Status = mepaxosproto.EXECUTED
		}
		stack = stack[0:l]
	}
	return true
