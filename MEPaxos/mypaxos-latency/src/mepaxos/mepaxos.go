...
/*func (r *Replica) isPassPropose() {
	for !r.Shutdown {
		time.Sleep(1e3) // 0.01 ms
		isPassProposeChan <- true
	}
}*/
        	...
        	if k==150 {//when it reached 15 seconds 
        		...
        		if replicaOrder {
        			for i := 0; i < r.N-1; i++ {
        				min := i
        				for j := i + 1; j < r.N-1; j++ {
        					if ave_latency[r.PreferredPeerOrder[j]] < ave_latency[r.PreferredPeerOrder[min]] {
        						min = j
        					}
        				}
...
func (r *Replica) run() {
	...
	for !r.Shutdown {
		...
		if r.DoPaxosState !=0 { //if not in transition state
			select {
...
func (r *Replica) handlePaxosPropose(propose *megenericsmr.PaxosPropose) {
	...
	batchSize := len(r.PaxosProposeChan) + 1
